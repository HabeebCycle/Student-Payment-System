...
if(isset($_POST['mosedt'])){
	...
	$id = $_POST['mosid'];
	mysqli_query($connection,"update admin set name='$name',phone='$phone',email='$email',info='$info' where id=$id");
	echo "<script>alert('Profile updated successfully');</script>";

}elseif(isset($_POST['mosdel'])){
	$id = $_SESSION['uid'];
	$pass = mysqli_fetch_array(mysqli_query($connection,"select password from admin where id=$id"))['password'];
	...
	if($pass == $pass1){
		if($pass2 == $pass3){
			mysqli_query($connection,"update admin set password='$pass2' where id=$id");
			  ...
			  <?php } ?>
                  ...
                  <div class="x_content">
                    <h3 class="text-muted font-13 m-b-30" >
					  ...
					  <?php
						$qnx = mysqli_query($connection,"select * from admin where id=".$_SESSION['uid']);
					  ...
					  ?>
						 ...
						 Your information/comment<textarea name="info" class="form-control" disabled><?php echo $dn['info']; ?></textarea>
						 <br/><br/>
						 <center>
									...
									<div class="modal-content">
										...
										<form class="form-horizontal form-label-left" name="mosque" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
											<div class="modal-body">
												...
												Your information/comment<textarea name="info" class="form-control"><?php echo $dn['info']; ?></textarea>
